...
# Выбор диапазона дат
start_date, end_date = st.sidebar.date_input('Выберите диапазон дат:', [data['date'].min(), data['date'].max()])
...
measurements_per_day = filtered_data.groupby('date')['value'].count().mean()
...
st.sidebar.markdown(f"**Минимальный пульс:** {min_pulse:.2f} уд/мин (Дата: {time_of_min_pulse})")
st.sidebar.markdown(f"**Максимальный пульс:** {max_pulse:.2f} уд/мин (Дата: {time_of_max_pulse})")
...
window_size = 7
filtered_data['rolling_avg'] = filtered_data['value'].rolling(window_size, min_periods=1).mean()

# Визуализация данных
fig = go.Figure()
fig.add_trace(go.Scatter(x=filtered_data['date'], y=filtered_data['value'], mode='markers', name='Пульс'))
fig.add_trace(go.Scatter(x=filtered_data['date'], y=filtered_data['rolling_avg'], mode='lines', name='Скользящее среднее'))

fig.update_layout(title='Динамика пульса', xaxis_title='Дата', yaxis_title='Пульс (уд/мин)')
